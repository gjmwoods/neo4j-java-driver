 ...
 */
package org.neo4j.connector.handlers.pulln;
...
public class BasicPullResponseHandler implements PullResponseHandler
{
    ...
    {
        if ( toRequest > 0 || toRequest == FetchSizeUtil.UNLIMITED_FETCH_SIZE )
        {
            request( toRequest );
            toRequest = 0;
        }
    ...
    {
        if ( toRequest == FetchSizeUtil.UNLIMITED_FETCH_SIZE )
        {
            return;
        }
        if ( toAdd == FetchSizeUtil.UNLIMITED_FETCH_SIZE )
        {
            // pull all
            toRequest = FetchSizeUtil.UNLIMITED_FETCH_SIZE;
            return;
        }
