...
import org.neo4j.connector.messaging.ValueUnpacker;
...
public class ValueUnpackerV1 implements ValueUnpacker
{
    ...
    private Value unpack() throws IOException
    {
        ...
        {
        case NULL:
            return Values.value( unpacker.unpackNull() );
        case BOOLEAN:
            return Values.value( unpacker.unpackBoolean() );
        case INTEGER:
            return Values.value( unpacker.unpackLong() );
        case FLOAT:
            return Values.value( unpacker.unpackDouble() );
        case BYTES:
            return Values.value( unpacker.unpackBytes() );
        case STRING:
            return Values.value( unpacker.unpackString() );
        case MAP:
        {
