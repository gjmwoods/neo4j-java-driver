...
public class AuthTokens
{
    ...
    public static AuthToken basic( String username, String password, String realm )
    {
        ...
        map.put( SCHEME_KEY, Values.value( "basic" ) );
        map.put( PRINCIPAL_KEY, Values.value( username ) );
        map.put( CREDENTIALS_KEY, Values.value( password ) );
        if ( realm != null )
        {
    ...
    public static AuthToken kerberos( String base64EncodedTicket )
    {
        ...
        map.put( SCHEME_KEY, Values.value( "kerberos" ) );
        map.put( PRINCIPAL_KEY, Values.value( "" ) ); // This empty string is required for backwards compatibility.
        map.put( CREDENTIALS_KEY, Values.value( base64EncodedTicket ) );
        return new InternalAuthToken( map );
    }
    ...
    public static AuthToken custom( String principal, String credentials, String realm, String scheme, Map<String, Object> parameters)
    {
        ...
        map.put( PRINCIPAL_KEY, Values.value( principal ) );
        map.put( CREDENTIALS_KEY, Values.value( credentials ) );
        if ( realm != null )
        {
            map.put( REALM_KEY, Values.value( realm ) );
        }
        if ( parameters != null )
        {
            map.put( PARAMETERS_KEY, Values.value( parameters ) );
        }
        return new InternalAuthToken( map );
    }
    ...
    {
        return new InternalAuthToken( singletonMap( SCHEME_KEY, Values.value( "none" ) ) );
    }
}
